...
def gama_tranfer(img):
    # img value [0,1] 
    gama = random.uniform(1,5)
    morethan_One_flag = random.random()
    if morethan_One_flag >0.5:
        img = img **gama
    else:
        img = img **(1/gama)
    return img
...
def get_colorization_data(data_raw, ab_thresh=5., p=.125, num_points=None):
    ...
    data_lab = rgb2lab(data_raw) # l [0-100] ab[-110,110]
    data['real_rgb'] = data_raw
    l_org = data_lab[:,[0,],:,:]
    ab_org = data_lab[:,1:,:,:]

    data['L'] = (l_org-50.)/100. *2. #[-1,1]
    data['ab'] = ab_org/110. #[-1,1]
    data['gama_L'] = gama_tranfer(l_org)
    data['ab_label'] = SoftEncodeAB(CIELAB())(ab_org)
...
class CIELAB:
    ...
    def __init__(self, gamut=None):
        ...
        # print('q_to_ab', self.q_to_ab)       # [313, 2]
